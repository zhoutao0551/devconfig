...
class AdminSite(object):
    ...
    def register(self, model_or_iterable, admin_class=object, **options):
        ...
        for model in model_or_iterable:
            if isinstance(model, ModelBase):
                ...
                # admin_class = type(str("%sAdmin" % model._meta.model_name), (admin_class,), options or {})
    ...
    def get_urls(self):
        ...
        urlpatterns += [
            re_path(
                _path if _path != '^xadmin/userprofile/(.+)/password/$' else '^userprofile/(.+)/password/$',
        ...
        for model, admin_class in iteritems(self._registry):
            ...
            urlpatterns += [
                ...
                re_path(r'^%s/' % (model._meta.model_name,), include(view_urls))
            ]
        return urlpatterns
